...
namespace nest
{
RecordablesMap< step_current_rec >
  step_current_rec::recordablesMap_; // rec from device

template <>
void
RecordablesMap< step_current_rec >::create()
{
...
nest::step_current_rec::Buffers_::Buffers_(
  step_current_rec& n ) // rec from device
  : logger_( n )
{
}

nest::step_current_rec::Buffers_::Buffers_( const Buffers_&,
  step_current_rec& n ) // rec from device
  : logger_( n )
{
}
...
void
nest::step_current_rec::Parameters_::get( DictionaryDatum& d ) const
{
  ( *d )[ "amplitude_times" ] =
...
void
nest::step_current_rec::Parameters_::set( const DictionaryDatum& d,
  Buffers_& b )
{
  const bool ut =
...
void
nest::step_current_rec::init_node_( const Node& proto )
{
...
void
nest::step_current_rec::init_state_( const Node& proto )
{
  const step_current_rec& pr = downcast< step_current_rec >( proto );
...
void
nest::step_current_rec::init_buffers_()
{
...
void
nest::step_current_rec::calibrate()
{
  B_.logger_.init();
  device_.calibrate();
}
...
void
nest::step_current_rec::update( Time const& origin,
  const long from,
  const long to )
{
...
void
nest::step_current_rec::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
