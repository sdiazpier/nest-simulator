...
{
  growth_curve_ =
    kernel().connection_manager.new_growth_curve( se.growth_curve_->get_name() );
  ...
  const SynapticElement& other )
{
  if ( this != &other )
  {
    ...
    GrowthCurve* new_gc =
      kernel().connection_manager.new_growth_curve( other.growth_curve_->get_name() );
    DictionaryDatum gc_parameters = DictionaryDatum( new Dictionary );
...
nest::SynapticElement::set( const DictionaryDatum& d )
{
  ...
  if ( d->known( names::growth_curve ) )
  {
    ...
    {
      growth_curve_ = kernel().connection_manager.new_growth_curve( growth_curve_name );
    }
  }
  growth_curve_->set( d );
